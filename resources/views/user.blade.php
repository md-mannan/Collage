...
@section('content')
    ...
    <div class="container mt-4 container mt-4 d-flex align-items-center justify-content-center">
        ...
        <div style="width:600px !important">
            ...
            <h3 class="mb-4">Update Student
                <small class="fs-5 ms-auto float-end text-info">
            ...
            <form id="updateForm" action="{{ route('student.update', $student->id) }}" method="POST">
                @csrf
                <input type="text" value="{{ $student->updated_at }}" name="updated_at" hidden>
                <div class="input-group mb-3">
                    <label for="studentId" class="input-group-text">ID</label>
                    <input class="form-control" disabled type="text" name="studentId" value="{{ $student->id }}">
                </div>
                <div class="input-group mb-3">
                    <label for="studentName" class="input-group-text">Name</label>
                    <input class="form-control" type="text" name="studentName"value="{{ $student->name }}">
                </div>
                <div class="input-group mb-3">
                    <label for="studentEmail" class="input-group-text">Email</label>
                    <input class="form-control" type="email" name="studentEmail"value=" {{ $student->email }}">
                </div>

                <div class="input-group mb-3">
                    <label for="studentAge" class="input-group-text">Age</label>
                    <input class="form-control" type="number" name="studentAge"value="{{ $student->age }}">
                </div>


                <div class="input-group mb-3">
                    <label for="studentGender" class="input-group-text">Gender:</label>
                    <select class="form-select text-capitalize" aria-label="Default select Gender" name="studentGender"
                        name="gender">
                ...
                <div class="input-group mb-3">
                    ...
                    <input type="text" class="form-control" name="studentCountry" value="{{ $student->country }}" />
                </div>
                <div class="input-group
                            mb-3">
                    <label for="studentCity" class="input-group-text">City:</label>
                    <input type="text" class="form-control" name="studentCity" value="{{ $student->city }}" />
                </div>
                <div class="input-group
                            mb-3">
                    <label for="studentAddress" class="input-group-text">Address:</label>
                    <textarea class="form-control" name="studentAddress" rows="3">{{ $student->address }}</textarea>
                </div>
                <div class="input-group mb-3">
